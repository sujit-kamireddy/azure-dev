...
`FOUNDRY_PROJECT_ENDPOINT` identifies the project that owns the named RLE and
provides the default fine-tuning resource: its `<account>.services.ai.azure.com`
host is converted to `<account>.openai.azure.com`. `--training-file` is required
and must point to a regular local training dataset file. The extension uploads it
to the selected fine-tuning resource with the `fine-tune` purpose, then uses the
returned file ID when it submits the job. `--validation-file` is optional and
follows the same local-file upload flow.

To target a different fine-tuning resource, pass `--endpoint` for one invocation
or set `AZD_AI_RLE_TRAIN_ENDPOINT` as the default override.
